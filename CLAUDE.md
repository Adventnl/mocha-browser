...
Milestones 1–25 are implemented (see docs/architecture/milestones.md for the authoritative list): engine pipeline (1–3), networking/navigation (4), events (5), a from-scratch JS interpreter + DOM bindings (6–7), subresources/images/forms (8–10), the minifb desktop window with chrome, tabs, and wheel scrolling (11–13), SQLite profile storage (14), cookies/web storage (15), security policy objects (16), IPC/multi-process and sandbox prototypes (17–18), DevTools snapshots (19), the compat/perf harness (20), real networking (21), real proportional page font metrics (22), forgiving fail-open HTML parsing that renders real content pages (23), the real-web CSS selector engine — combinators, attribute selectors, and structural pseudo-classes (24), and a multi-subsystem expansion (25): more JS array/string methods + `for…of`/`for…in`, HTML table sections + `<select>`/`<textarea>`, CSS `@media` width queries + `font-family`, and `Content-Encoding: deflate`. Recent additions:
