...
not a network process, not a GPU process, and not an OS sandbox. The renderer
binary currently calls `mocha_engine::render_url` directly for
`RenderDocument`, which means it can still perform file/http loads with the same
OS privileges as the parent process. M18 is expected to move toward a real
